...
title=$(grep -F "標題" $dirsub/LATEST/Translated.ass | grep -F "Dialogue" | awk '!/bord0/' | sed $SEDOPTION_L 's/.*,,0,0,0,,//' | grep -v '櫻桃小丸子' | rev | cut -d '}' -f1 | rev )
#| uniq | sed $SEDOPTION_L 's/ //g' | head -n -2 | awk 'NF' | tr '\r' ',' | tr '\n' ',' | sed $SEDOPTION_L 's/,,/,/g' | sed $SEDOPTION_L 's/,/ + /g' | sed 's/.\{2\}$//')
