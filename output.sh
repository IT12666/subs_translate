...
echo addtxt="GFilePath = \"$dirsub\"" && sed -i "14s/.*/$addtxt/"  $dirsub/odysee.py
...
echo addtxt="GThumbnailFileURL = \"$cover\"" && sed -i "26s/.*/$addtxt/"  $dirsub/odysee.py
...
echo addtxt="GDescription = \"$desc\"" && sed -i "44s/.*/$addtxt/"  $dirsub/odysee.py
#addtxt="GTagFile = \"\"" && sed -i "47s/.*/$addtxt/"  $dirsub/odysee.py
addtxt="GTags = \[$(grep -F $epname".search=" $setup | cut -d "=" -f2)\]" && sed -i "50s/.*/$addtxt/"  $dirsub/odysee.py
