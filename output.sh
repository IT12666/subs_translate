...
while read line; do source=$(echo "$line" | rev | cut -d'|' -f 2 | rev | awk '{gsub( /[(*|`$)]/, "\\\\&"); print $0}') && result=$(echo "$line" | cut -d'|' -f 2 | awk '{gsub( /[(*|`$)]/, "\\\\&"); print $0}') && echo sed $SEDOPTION "s!$source!$result!g"  $dirsub/LATEST/Translated.ass; done < "$currdir/_ESSENTIAL/Replacement/$(basename $dirsub)/Style.txt"
