...
while read line; do source=$(echo "$line" | rev | cut -d'|' -f 2 | rev) && result=$(echo "$line" | cut -d'|' -f 2) && echo sed $SEDOPTION "s!$source!$result!g"  $dirsub/LATEST/Translated.ass; done < cat -v "$currdir/_ESSENTIAL/Replacement/$(basename $dirsub)/Style.txt"
